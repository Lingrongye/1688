import requests
from utils import get_shop_info
...
def get_shop_goods(url,num_of_data=100,file_path='',tag=''):
	...
	params = {
		...
		'appKey': '12574478',
		# 't': '1730303434192',
		# 'sign': 'e531e89638b2b286b823a1f720dade19',
	...
	memberId = get_shop_info(url,cookies,headers)
	number_of_data = 0
	for page_num in range(1,100):
		# if num_of_data >num_of_data:
		# 	break
		data = {}
		data_para =  '{"componentKey":"Wp_pc_common_offerlist","params":"{\\"memberId\\":\\"%s\\",' \
		             '\\"appdata\\":{\\"sortType\\":\\"wangpu_score\\",\\"sellerRecommendFilter\\":false,\\"mixFilter\\":false,' \
		             '\\"tradenumFilter\\":false,\\"quantityBegin\\":null,\\"pageNum\\":%s,\\"count\\":30}}"}' % (memberId,page_num)
		data['data'] = data_para
		...
		try:
			...
			items = response.json()['data']['content']['offerList']
...
get_shop_goods('https://shop17755e906s484.1688.com/page/index.html?spm=a261y.7663282.wp_pc_common_header_companyName_undefined.0',5,'test.csv','test')